...
const PROJECTS: ProjectConfig[] = [
  {
    ...
    milestones: [
      ...
      { title: 'Roadmap panel — auto-derived from GitHub + Brain', detail: 'API route fetching commits + deploys + Brain items · project drill-down', status: 'done', date: '30 Mar' },
  ...
  {
    id: 'coolbrick-agent',
    repo: 'AlanCoolbrick/coolbrick-agent',
    category: 'Core',
    label: 'Coolbrick Brain',
  ...
  {
    id: 'coolbrick-brain-app',
    repo: 'AlanCoolbrick/coolbrick-brain-app',
    category: 'Core',
  ...
  {
    id: 'rooms-tracker',
    repo: 'AlanCoolbrick/rooms-tracker',
    category: 'Tools',
    label: 'Rooms Tracker',
    ...
    milestones: [
      ...
      { title: 'Being absorbed into ALAN OS Property panel', detail: 'PropertyPanel.tsx replaces rooms-tracker functionality', status: 'active' },
    ],
  },
  {
    id: 'coolbrick-hub',
    repo: 'AlanCoolbrick/coolbrick-hub',
    category: 'Tools',
    label: 'Coolbrick Hub',
  ...
  {
    id: 'openclaw-gemini',
    repo: 'AlanCoolbrick/openclaw-gemini',
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (pid) {
    ...
    const c = commits[cfg.repo] || [];
    const d = deploys[cfg.repo] || null;
  ...
  const projects: ProjectSummary[] = PROJECTS.map((p) => {
    const c = commits[p.repo] || [];
    const d = deploys[p.repo] || null;
...
async function fetchCommits(token: string, since: string, pp: number): Promise<Record<string, CommitInfo[]>> {
  ...
  await Promise.all(PROJECTS.map(async (p) => {
    try {
      const res = await fetch(`https://api.github.com/repos/${p.repo}/commits?since=${since}&per_page=${pp}`, {
        headers: { Authorization: `token ${token}`, Accept: 'application/vnd.github.v3+json' },
      });
      if (res.ok) {
        const d = await res.json();
        r[p.repo] = d.map((c: any) => ({ sha: c.sha.slice(0, 7), message: c.commit.message.split('\n')[0].slice(0, 120), date: c.commit.committer.date, author: c.commit.author.name }));
      } else r[p.repo] = [];
    } catch { r[p.repo] = []; }
  }));
  return r;
