...
const PROJECTS: ProjectConfig[] = [
  {
    ...
    milestones: [
      { title: 'Session 1: Foundation — convert to Next.js/TypeScript', detail: 'Upgrade deps · copy 10 components + 4 API routes from Brain · FocusPanel wrapper · ~2 hrs', status: 'done', date: '30 Mar' },
      { title: 'Session 2: Shell integration — build ALAN OS chrome as React', detail: 'Convert patch files to components · wire panel switching · global Cmd+K + ContextWing · ~1.5 hrs', status: 'done', date: '30 Mar' },
      { title: 'Session 3: Polish + deploy', detail: 'Delete old files · Vercel env vars · smoke test all panels · update AGENTS.md · ~1 hr', status: 'active' },
      { title: 'CSS dark-theme adaptation', detail: 'Brain light theme → ALAN OS dark theme · biggest risk in the migration', status: 'active' },
  ...
  },
  {
    ...
    milestones: [
      ...
      { title: 'Scheduler dedup guard', detail: 'Idempotency lock via claim_scheduler_lock() RPC — prevents duplicate cron fires', status: 'done', date: '24 Mar' },
      { title: '/save source field fix', detail: 'Was writing claude_session instead of whatsapp — fixed 30 Mar', status: 'done', date: '30 Mar' },
    ],
  },
  {
    ...
    milestones: [
      ...
      { title: 'Migration to ALAN OS', detail: 'Session 1+2 complete (foundation + shell). Session 3 (polish) in progress. Brain Dashboard kept as admin fallback.', status: 'active' },
    ],
  },
  {
