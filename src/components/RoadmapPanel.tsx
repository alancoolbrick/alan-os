...
export default function RoadmapPanel() {
  ...
  if (error && !data) return (
    ...
    <div style={emptyStyle}>Failed to load roadmap</div></>
  );
  if (!data) return (
  ...
  return (
    <>
      <div className="mode-header"><div className="mode-title">Roadmap</div>
        <div className="mode-sub">{projects.length} projects &middot; {active} active</div></div>
      <div className="g3">
              ...
              {cp.map((p) => (
                <div key={p.id} className="prow" style={{ gap: 10, alignItems: 'center' }} onClick={() => setSelectedProject(p.id)}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 9, color: 'var(--dim)', fontFamily: mono, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>{p.description}</div>
                  </div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexShrink: 0 }}>
                    ...
                    <span style={{ fontSize: 10, color: 'var(--dim)' }}>&rarr;</span>
                  </div>
                </div>
              ))}
            </div>
...
function DetailView({ detail, loading, onBack }: { detail: ProjectDetail | null; loading: boolean; onBack: () => void }) {
  ...
  if (loading || !detail) return (
    <><div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
      <button onClick={onBack} style={backBtn}>&larr; Back</button></div>
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 2 }}>
        ...
        <div className="mode-header" style={{ marginBottom: 0 }}><div className="mode-title">{detail.label}</div><StatusPill status={detail.status} color={detail.color} /></div>
      </div>
      <div style={{ fontSize: 10, color: 'var(--dim)', fontFamily: mono, marginBottom: 10, paddingLeft: 40 }}>{detail.description}</div>
      <div className="g3">
        <div className="card"><div className="card-lbl">Progress</div><div className="card-val teal">{pct}%</div><div className="card-note">{done} of {total} milestones</div>
          <div className="bar" style={{ marginTop: 6 }}><div className="bar-fill" style={{ width: pct + '%', background: 'var(--teal)' }} /></div></div>
        <div className="card"><div className="card-lbl">Commits (7d)</div><div className={`card-val ${detail.commitCount > 0 ? 'gold' : 'rose'}`}>{detail.commitCount}</div><div className="card-note">{timeAgo(detail.lastCommit) || 'none'}</div></div>
        <div className="card"><div className="card-lbl">Brain Items</div><div className="card-val gold">{detail.brainItems}</div><div className="card-note">{detail.blockedTasks} focus/next</div></div>
      </div>
      {detail.decision && (<div style={decisionBox}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
        <a href={detail.githubUrl} target="_blank" rel="noopener noreferrer" style={linkBtn}>GitHub &rarr;</a>
        {detail.deployUrl && <a href={`https://${detail.deployUrl}`} target="_blank" rel="noopener noreferrer" style={linkBtn}>Live site &rarr;</a>}
      </div>
      {doneMs.length > 0 && (<>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}><PhaseLabel label="COMPLETE" color="var(--teal)" /></div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14 }}>{doneMs.map((m, i) => <MilestoneCard key={i} m={m} />)}</div>
      </>)}
      {activeMs.length > 0 && (<>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}><PhaseLabel label="IN PROGRESS" color="var(--gold)" /></div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14 }}>{activeMs.map((m, i) => <MilestoneCard key={i} m={m} />)}</div>
      </>)}
      {plannedMs.length > 0 && (<>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}><PhaseLabel label="PLANNED" color="var(--dim)" /></div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 14 }}>{plannedMs.map((m, i) => <MilestoneCard key={i} m={m} />)}</div>
      </>)}
      {detail.brainItemList.length > 0 && (<>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 14 }}>
          {detail.brainItemList.map((item) => (
            <div key={item.id} className="task"><div className="tcheck" /><div className="task-body">
              <div className="task-txt">{item.title}<span className={`tag ${item.type === 'project' ? 'f' : 'o'}`}>{item.type}</span></div>
              <div className="task-meta">{item.status} &middot; {timeAgo(item.updated_at)}</div>
            </div></div>
          ))}
        </div>
      </>)}
      {detail.commits.length > 0 && (<>
        <div className="sec" style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }} onClick={() => setShowCommits(!showCommits)}>
          <span>{showCommits ? '\u25be' : '\u25b8'}</span> Activity &mdash; {detail.commits.length} commits (7d)
        </div>
        {showCommits && (<div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {detail.commits.map((c, i) => (
            <div key={c.sha + i} style={commitRow}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 2 }}><span style={cmeta}>{c.author}</span><span style={cmeta}>{formatDate(c.date)}</span></div>
            </div>
          ))}
        </div>)}
      </>)}
...
function MilestoneCard({ m }: { m: Milestone }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 7, background: 'var(--s1)', border: '1px solid var(--b1)', opacity: m.status === 'done' ? 0.7 : 1 }}>
      <span style={{ fontSize: 14, flexShrink: 0, marginTop: 1 }}>{icon}</span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12, color: 'var(--mid)', fontWeight: 500, lineHeight: 1.4 }}>{m.title}</div>
        <div style={{ fontSize: 9, color: 'var(--dim)', fontFamily: mono, marginTop: 2, lineHeight: 1.4 }}>{m.detail}{m.date ? ` \u00b7 ${m.date}` : ''}</div>
      </div>
    </div>
  );
}
...
function PhaseLabel({ label, color }: { label: string; color: string }) {
  return (<span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: 4, fontSize: 9, fontFamily: mono, fontWeight: 700, letterSpacing: '.08em',
    background: color === 'var(--teal)' ? 'rgba(61,232,176,.1)' : color === 'var(--gold)' ? 'rgba(232,176,75,.1)' : 'var(--s2)',
    color, border: `1px solid ${color === 'var(--teal)' ? 'rgba(61,232,176,.2)' : color === 'var(--gold)' ? 'rgba(232,176,75,.2)' : 'var(--b2)'}` }}>{label}</span>);
}
...
function StatusPill({ status, color }: { status: string; color: string }) {
  ...
  return (<span style={{ display: 'inline-flex', alignItems: 'center', padding: '2px 8px', borderRadius: 20, fontSize: 8, fontFamily: mono, background: st.bg, border: `1px solid ${st.border}`, color: st.text, whiteSpace: 'nowrap' }}>{status}</span>);
}
...
const decisionBox: React.CSSProperties = { padding: '12px 16px', borderRadius: 8, marginBottom: 8, background: 'rgba(232,176,75,.06)', border: '1px solid rgba(232,176,75,.18)' };
